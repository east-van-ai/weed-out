...
from weed_out.cli import EXIT_ARGPARSE, EXIT_ERROR, EXIT_OK

# ---------- grammar ----------


def test_cli_bare_prints_banner_whatever_stdin_is(run_cli):
    """A bare word is a question, and stdin decides none of it.

    run_cli attaches a pipe to stdin, the same shape cron or CI hands a
    process, so this is the launch context that used to exit 1.
    """
    result = run_cli([], input_text="")
    assert result.returncode == EXIT_OK
    assert "~~~ weed-out ~~~" in result.stdout
    assert "Exit codes:" in result.stdout
    assert result.stderr == ""


def test_cli_bare_command_word_prints_its_own_doc(run_cli):
    """A command word and nothing else at all is a question too."""
    for command in ("delete", "trash", "tree"):
        result = run_cli([command], input_text="")
        assert result.returncode == EXIT_OK
        assert f"~~~ weed-out {command} ~~~" in result.stdout
...
def test_cli_missing_path_is_usage_error(run_cli):
    """Once any other token is present, help would hide the mistake: exit 1,
    ours, with the command's own usage line."""
    result = run_cli(["delete", "--keep", "keep.md"], input_text="")
    assert result.returncode == EXIT_ERROR
    assert "delete needs PATH" in result.stderr
    assert "Usage: weed-out delete" in result.stderr


def test_cli_path_after_flags_is_rejected(run_cli, sample_tree):
    """Positions are decided, not inferred: a PATH argparse back-fills from
    after the flags (3.12+) is discarded, on every interpreter alike."""
    result = run_cli(["delete", "--keep", "keep.md", str(sample_tree)], input_text="")
    assert result.returncode == EXIT_ERROR
    assert "delete needs PATH" in result.stderr
    assert (sample_tree / "drop.txt").exists()


def test_cli_stray_token_after_path_is_rejected(run_cli, sample_tree):
    result = run_cli(["tree", str(sample_tree), "extra"], input_text="")
    assert result.returncode == EXIT_ERROR
    assert "tree takes nothing after PATH: 'extra'" in result.stderr
    assert "Usage: weed-out tree" in result.stderr
...
def test_cli_abbreviated_commit_flag_is_rejected(run_cli, sample_tree):
    ...
    """
    result = run_cli(
        ["delete", str(sample_tree), "--keep", "keep.md", "--com"], input_text=""
    )
    assert result.returncode == EXIT_ARGPARSE
    assert (sample_tree / "drop.txt").exists()
...
def test_cli_abbreviated_read_only_flag_is_rejected(run_cli, sample_tree):
    """The rule covers the whole flag surface, not just the destructive one."""
    result = run_cli(
        ["delete", str(sample_tree), "--keep", "keep.md", "--dry-ru"], input_text=""
    )
    assert result.returncode == EXIT_ARGPARSE
...
def test_cli_dry_run_explicit_flag_matches_default(run_cli, sample_tree):
    ...
    assert explicit_result.returncode == EXIT_OK
...
def test_cli_dry_run_footer_names_the_disposal(run_cli, sample_tree):
    ...
    assert delete_result.returncode == EXIT_OK
    assert trash_result.returncode == EXIT_OK
...
def test_cli_tree_and_dry_run_agree_about_a_named_file_sibling(
    ...
    assert tree.returncode == EXIT_OK
    assert dry_run.returncode == EXIT_OK
